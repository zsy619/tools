...
func main() {
	...
	client := xcas.NewClient(&xcas.Options{
		URL: url,
	})
...
type templateBinding struct {
	Username   string
	Attributes xcas.UserAttributes
}

func (h *myHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if !xcas.IsAuthenticated(r) {
		xcas.RedirectToLogin(w, r)
		return
	}

	if r.URL.Path == "/logout" {
		xcas.RedirectToLogout(w, r)
		return
	}
	...
	binding := &templateBinding{
		Username:   xcas.Username(r),
		Attributes: xcas.Attributes(r),
	}
