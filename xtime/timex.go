...
// GetDayBeginMoment1 将 Unix 秒时间戳转换为当天开始时刻。
...
// GetDayEndMoment1 将 Unix 秒时间戳转换为当天结束时刻。
...
// GetDayElapsedS 返回指定时间距当天开始时刻的秒数。
...
// GetDayElapsedMs 返回指定时间距当天开始时刻的毫秒数。
...
// GetDayElapsedUs 返回指定时间距当天开始时刻的微秒数。
...
// GetHoursBtwTs 返回两个 Unix 时间戳之间向下取整的小时数。
...
// IsSameHour 判断两个 Unix 时间戳是否同一小时。
...
// IsSameMinute 判断两个 Unix 时间戳是否同一分钟。
