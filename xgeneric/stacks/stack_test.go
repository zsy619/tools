...
func ExampleNewStack() {
	...
	eventBus := NewStack[*string]()
	fmt.Println(eventBus.IsEmpty())

	eventBus.Push(xconv.ToPtr("Sig No 109"))
	eventBus.Push(xconv.ToPtr("Sig No 282"))
...
func ExampleNewStackSize() {
	// event bus send event as FILO mode
	eventBus := NewStackSize[*string](10)
	fmt.Println(eventBus.IsEmpty())
