...
// Contains 在切片中查找 val 首次出现的下标。
// 参数：collection 为输入切片，val 为要查找的元素。
...
// FindLastIndexOf 根据谓词函数从尾部向前查找最后一个满足条件的元素，并返回其下标。
// 参数：collection 为待搜索的切片，predicate 为判断元素是否满足条件的函数。
// 返回：匹配的元素、其下标以及 true；若未找到则返回 T 的零值、-1 与 false。
...
// FindOrElse 根据谓词函数查找第一个满足条件的元素；若未找到则返回给定的回退值。
...
// Min 返回切片中的最小值。
// 参数：collection 为输入切片（元素必须满足 xgeneric.Ordered 约束）。
// 返回：切片中的最小值；当切片为空时返回 T 的零值。
func Min[T xgeneric.Ordered](collection []T) T {
	var min T
...
func Max[T xgeneric.Ordered](collection []T) T {
	var max T
...
// Sample 从切片中随机返回一个元素。
// 参数：collection 为输入切片。
// 返回：随机选中的元素；当切片为空时返回 xgeneric.Empty[T]()（即 T 的零值）。使用 math/rand 进行随机抽样。
...
// 返回：长度最多为 count 的新切片，每个元素互不相同；当 count 大于切片长度时仅返回切片全部元素。使用 math/rand 进行抽样。
