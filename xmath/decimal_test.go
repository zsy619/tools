...
func TestDecimal_MinDecimal(t *testing.T) {
	...
	for _, test := range testCases {
		...
		got := MinDecimal(decimalInput[0], decimalInput[1:]...)
...
func TestDecimal_MaxDecimal(t *testing.T) {
	// the first element in the array is the expected answer, rest are inputs
	testCases := [][]float64{
		{0, 0},
		{1, 1},
	...
	for _, test := range testCases {
		...
		got := MaxDecimal(decimalInput[0], decimalInput[1:]...)
		if !got.Equal(expectedDecimal) {
			t.Errorf("Expected %v, got %v, input=%+v", expectedDecimal, got,
				decimalInput)
		}
